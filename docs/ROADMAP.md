...
- **Manejo robusto de un frame de audio llegando mientras hay una confirmación pendiente**: `pedir_confirmacion` hoy solo espera un mensaje JSON, y un frame binario ahí lo rompe con un error (no silencioso, pero tampoco elegante). Rough edge conocido para la Fase 7.
...
- **Observabilidad real** (métricas de latencia expuestas, no solo logs): también Fase 7/`devops-engineer` — evaluar primero si algo más simple que un stack Prometheus/Grafana alcanza para un sistema mono-usuario.
- **Persistencia de la conversación entre reinicios del backend**: hoy `history` vive en memoria por conexión WS — un restart del servidor o una reconexión pierden el contexto de la sesión anterior.

### Contratos compartidos
- **`packages/rag-engine/`, `packages/voice-pipeline/` y `packages/shared-contracts/` siguen vacíos a propósito** — se justifican recién cuando exista un segundo consumidor real (el candidato más concreto: el frontend de la Fase 6 necesitando los schemas de mensajes WS/tools de `shared-contracts` en TypeScript, para no duplicarlos a mano y que diverjan del lado Python).
